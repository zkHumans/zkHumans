  ...
  return {
    ...
    async listen() {
      ...
      app.post('/castVote', (req, res) => {
        ...
        const jsonVote = req.body;
        if (validateJSONVote(jsonVote)) {
          const isValidOrError = validateVote(jsonVote, VotePool, voterData);
...
function validateVote(
  ...
  const vote = Vote.fromJSON(jsonVote);

  const isVoter = voterData.find((v) =>
